        ...
        <div class="card-footer bg-white p-3">
            @if($phiu['TrangThai'] == 'Chờ duyệt')
                <form action="{{ route('quanly.kiemke.tuchoi', $phiu['MaPhieuKiemKe']) }}" method="POST" class="row g-2 align-items-center mb-3 border-bottom pb-3">
                    @csrf
                    <div class="col-md-9">
                        <input type="text" name="ghi_chu_tu_choi" class="form-control form-control-sm" placeholder="⚠️ Bắt buộc nhập ghi chú lý do chênh lệch vật lý trước khi bấm từ chối..." required>
                    </div>
                    <div class="col-md-3">
                ...
                <div class="text-end">
                    @if($phiu['isFullyMatched'])
                        <form action="{{ route('quanly.chotca', $phiu['MaPhieuKiemKe']) }}" method="POST" class="d-inline">
                            @csrf
                            <button type="submit" class="btn btn-success btn-sm px-4 fw-bold shadow-sm">Xác Nhận & Chốt Ca Bếp</button>
                        </form>
                    @else
                        <button type="button" class="btn btn-secondary btn-sm px-4 fw-bold shadow-sm" disabled style="cursor: not-allowed;">
                            🔒 Khóa Chốt Ca (Số liệu chưa khớp)
                        </button>
                        <small class="text-danger d-block mt-2 font-monospace fw-bold">
                            ⚠️ Hệ thống đã tự động khóa chốt ca do phát hiện chênh lệch vật lý!
                        </small>
                    @endif
                </div>
            
            @elseif($phiu['TrangThai'] == 'Nháp')
                <div class="alert alert-danger text-start mb-0 shadow-sm border-start border-4 border-danger">
                    <h6 class="fw-bold text-danger mb-1">❌ BÁO CÁO BỊ TỪ CHỐI </h6>
                    <p class="mb-0 small text-dark">
                        📌Nội dung ghi chú lý do sai lệch: <span class="text-danger fw-bold italic">"{{ $phiu['GhiChu'] ?? 'Số liệu kiểm kê không khớp với thực tế kiểm đếm tại khu vực bếp.' }}"</span>
                    </p>
                </div>
            @elseif($phiu['TrangThai'] == 'Đã duyệt')
                <div class="text-end">
                    <span class="text-success fw-bold">✓ Đã phê duyệt phiếu kiểm kê cuối ngày</span>
                </div>
            @endif
        </div>
    </div>
