...
class KiemKeKhoChinhController extends Controller
{
    ...
    public function store(Request $request)
    {
        ...
        return "<script>alert('Gửi dữ liệu kiểm kê kho chính thành công!'); window.location.href='" . route('dashboard') . "';</script>";
    }
    ...
    public function danhSachDuyet(Request $request)
    {
        ...
        foreach ($phieus as $p) {
            $details = DB::table('ChiTietPhieuKiemKeDinhKy')->where('MaPhieuKiemKe', $p->MaPhieuKiemKe)->get();
            $biLech = false;
            $editedLotsOfPhieu = session()->get("edited_lots.{$p->MaPhieuKiemKe}", []);
    ...
    public function hieuChinhPhieu(Request $request, $maPhieu)
    {
        ...
        $editedLots = session()->get("edited_lots.{$maPhieu}", []);
        if (in_array($maLo, $editedLots)) {
            return redirect()->back()->with('status', '⚠️ Hành động bị chặn: Lô hàng này đã được hiệu chỉnh số liệu một lần trước đó rồi!');
        }
    ...
    {
        $detailsLech = DB::table('ChiTietPhieuKiemKeDinhKy')
            ->where('MaPhieuKiemKe', $maPhieu)
            ->where('ChenhLech', '!=', 0)
            ->get();
    ...
    public function taoGiaiTrinh(Request $request, $maPhieu)
    {
       $request->validate([
        ...
        return redirect()->route('giai-trinh.index')->with('status', 'Xác nhận đối soát thành công! Đã lưu vết và tự động gửi phiếu giải trình thất thoát lên hệ thống thẩm định.');
    }
